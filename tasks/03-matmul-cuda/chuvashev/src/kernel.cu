...
void run_matrix_mult_gpu_ver_2(const std::vector<__half> &input,
  ...
  std::cout << end_time << "\t" << "GB\s: " << n * n * (2.0f * sizeof(__half) + 1.0f*sizeof(float))  / (1024.0f * 1024.0f * 1024.0f * end_time) << std::endl;
...
void run_wmma(const std::vector<__half> &intput, std::vector<float> &output,
              const std::size_t n) {
