...
void test_simd_correctness() {
    ...
    std::cout << "Проверяем граничные случаи (размеры, кратные 8 и некратные):\n\n";
    
    // Критические размеры для проверки
    std::vector<std::size_t> test_sizes = {1, 2, 3, 4, 5, 6, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128, 129};
...
void debug_single_test(std::size_t n) {
    ...
    for (std::size_t j = 0; j < std::min(n, static_cast<std::size_t>(10)); ++j) {
        std::cout << "  Элемент " << j << ": input=" << std::setprecision(4) << matrix[j]
                  << ", scalar=" << scalar_result[j]
                  << ", SIMD=" << simd_result[j]
...
int main(int argc, char *argv[]) {
    ...
    try {
        if (argc != 2) {
            ...
            std::cerr << "       " << argv[0] << " --debug N  (отладка для размера N)\n";
            return EXIT_FAILURE;
        }
